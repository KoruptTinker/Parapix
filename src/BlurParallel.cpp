...
void blurParallel(char inputName[],char outputName[]) 
{
    ...
    #pragma omp parallel shared(maskedImgR,maskedImgG,maskedImgB) private(i,j,itImageX,itImageY,itMaskX,itMaskY,averageR,averageG,averageB)
    {
        ...
        for( i=0 ; i < height ; i++)
        {
            for( j=0 ; j < width ; j++)
            {
                ...
                averageG = 0;
                averageB = 0;
                for(itImageX=i-maskDim/2,itMaskX=0;itImageX<=(i+maskDim/2)&&itMaskX<maskDim;itImageX++,itMaskX++)
                {
                    if(itImageX>=0 && itImageX<=height)
                    {
                        png_bytep row = row_pointers[itImageX];
                        for(itImageY=j-maskDim/2,itMaskY=0;itImageY<=(j+maskDim/2)&&itMaskY<maskDim;itImageY++,itMaskY++)
                        {
